...
ifdef CONFIG_TARGET_hisilicon
  ...
  TARGET_CFLAGS += -Wl,-rpath-link=$(STAGING_DIR)/usr/lib -Wl,-rpath-link=$(shell echo $(CONFIG_LIBGCC_ROOT_DIR))/lib
  TARGET_LDFLAGS += -L$(STAGING_DIR)/usr/lib 
endif

ifdef CONFIG_TARGET_quanzhi
  TARGET_CFLAGS += -Wl,-rpath-link=$(shell echo $(CONFIG_LIBGCC_ROOT_DIR))/lib
...
ifeq ($(findstring yodamk,$(PKG_DIR_NAME)),)
ROKID_BUILD:=y
endif
...
PKG_INSTALL_STAMP:=$(if $(BUILD_VARIANT),$(BUILD_VARIANT),default).install
endif
...
ifdef USE_SOURCE_DIR
  define Build/Prepare/Default
	rm -rf $(PKG_BUILD_DIR)
	$(if -e $(wildcard $(USE_SOURCE_DIR)/*),,@echo "Error: USE_SOURCE_DIR=$(USE_SOURCE_DIR) path not found"; false)
...
ifeq ($(ROKID_BUILD),y)
	...
	rm -rf $(TMP_DIR)/stage-$(1)
	mkdir -p $(TMP_DIR)/stage-$(1)/host $(STAGING_DIR)/packages $(STAGING_DIR_HOST)/packages
	$(foreach hook,$(Hooks/InstallDev/Pre),\
		$(call $(hook),$(TMP_DIR)/stage-$(1),$(TMP_DIR)/stage-$(1)/host)$(sep)\
	)
	$(call Build/InstallDev,$(TMP_DIR)/stage-$(1),$(TMP_DIR)/stage-$(1)/host)
	$(foreach hook,$(Hooks/InstallDev/Post),\
		$(call $(hook),$(TMP_DIR)/stage-$(1),$(TMP_DIR)/stage-$(1)/host)$(sep)\
	)
	if [ -f $(STAGING_DIR)/packages/$(STAGING_FILES_LIST) ]; then \
	...
	if [ -d $(TMP_DIR)/stage-$(1) ]; then \
		(cd $(TMP_DIR)/stage-$(1); find ./ > $(TMP_DIR)/stage-$(1).files); \
		$(call locked, \
			mv $(TMP_DIR)/stage-$(1).files $(STAGING_DIR)/packages/$(STAGING_FILES_LIST) && \
			$(CP) $(TMP_DIR)/stage-$(1)/* $(STAGING_DIR)/; \
		,staging-dir); \
	fi
	rm -rf $(TMP_DIR)/stage-$(1)
	touch $$@
...
ifeq ($(PKG_INSTALL_STAMP),default.install)
	@touch $(PKG_INFO_DIR)/$(1).$(PKG_INSTALL_STAMP).clean
	@echo "$(filter-out essential,$(PKG_FLAGS))" > $(PKG_INFO_DIR)/$(1).$(PKG_INSTALL_STAMP).flags
else
