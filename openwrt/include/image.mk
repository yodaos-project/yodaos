...
ifneq ($(CONFIG_BINARY_PREFIX),"")
  IMG_PREFIX:=$(CONFIG_BINARY_PREFIX)
else
...
SQUASHFSCOMP := lz4 -Xhc -no-xattrs -no-exports -always-use-fragments
#SQUASHFSCOMP := lzma
...
define Image/mkfs/squashfs
	$(STAGING_DIR_HOST)/bin/mksquashfs4 $(TARGET_DIR) $(KDIR)/root.squashfs -noappend -no-exports -no-sparse -no-xattrs -all-root -comp $(SQUASHFSCOMP) $(SQUASHFSOPT) -processors $(if $(CONFIG_PKG_BUILD_JOBS),$(CONFIG_PKG_BUILD_JOBS),8)
ifeq ($(CONFIG_TARGET_ROOTFS_SQUASHFS_MIX),y)
	$(CP) ./ubinize_squash.cfg $(KDIR)
	sed 's;ROOT_SQUASHFS_IMAGE;$(KDIR)/root.squashfs;' $(KDIR)/ubinize_squash.cfg > $(KDIR)/ubinize_build.cfg
	sed 's;SQUASHFS_IMAGE_SIZE;$(CONFIG_TARGET_ROOTFS_SQUASHFS_MIX_SIZE)MiB;' $(KDIR)/ubinize_build.cfg > $(KDIR)/ubinize_ok.cfg
	$(STAGING_DIR_HOST)/bin/ubinize -o $(KDIR)/root.squash -m 0x800 -p 0x20000 -s 2048 $(KDIR)/ubinize_ok.cfg
endif
endef
...
ifneq ($(CONFIG_TARGET_APP_UBIFS),)
define Image/mkfs/app/generate
	$(CP) ./ubinize_app.cfg $(KDIR)
	( cd $(KDIR); \
...
define Image/mkfs/app
	echo $(TARGET_DIR)
	mkdir -p $(TARGET_APP_DIR)
	mkdir -p $(TARGET_DIR)/data/custom
	ls $(TARGET_DIR)/data/* -lah
	cp -rf $(TARGET_DIR)/data/*  $(TARGET_APP_DIR)/
	rm -rf $(TARGET_DIR)/data/*
       ifneq ($($(PROFILE)_UBIFS_OPTS)$(UBIFS_OPTS),)
		$(STAGING_DIR_HOST)/bin/mkfs.ubifs \
		$(if $($(PROFILE)_APP_OPTS), \
			$(shell echo $($(PROFILE)_APP_OPTS)), \
			$(shell echo $(APP_OPTS)) \
		) \
	...
	$(if $(wildcard ./ubinize_app.cfg),$(call Image/mkfs/app/generate,))
	$(if $(wildcard ./ubinize-overlay.cfg),$(call Image/mkfs/app/generate,-overlay))
    endif
$(if $(wildcard ./ubinize_app.cfg),$(call Image/Build,app))
endef
endif
...
ifeq ($(CONFIG_TARGET_OVERLAY_UBIFS),y)
	$(CP) ./ubinize_ubifs_overlay.cfg $(KDIR)/ubinize$(1).cfg
else
	...
	echo $(TARGET_DIR)
		...
		$(STAGING_DIR_HOST)/bin/mkfs.ubifs \
			...
			$(if $(CONFIG_TARGET_UBIFS_COMPRESSION_XZ),--force-compr=xz) \
			$(if $(shell echo $(CONFIG_TARGET_UBIFS_JOURNAL_SIZE)),--jrn-size=$(CONFIG_TARGET_UBIFS_JOURNAL_SIZE)) \
			--squash-uids \
			-o $(KDIR)/root.ubifs \
			-d $(TARGET_DIR)
	...
	$(call Image/Build,ubifs)
        ...
        ifneq ($($(PROFILE)_UBI_OPTS)$(UBI_OPTS),)	    
ifeq ($(CONFIG_TARGET_OVERLAY_UBIFS),y)
		$(if $(wildcard ./ubinize_ubifs_overlay.cfg),$(call Image/mkfs/ubifs/generate,))
		echo "ubinize_ubifs_overlay.cfg is used"
else
		$(if $(wildcard ./ubinize.cfg),$(call Image/mkfs/ubifs/generate,))
		echo "ubinize.cfg is used"
endif
		$(if $(wildcard ./ubinize-overlay.cfg),$(call Image/mkfs/ubifs/generate,-overlay))

        endif
ifeq ($(CONFIG_TARGET_OVERLAY_UBIFS),y)
	$(if $(wildcard ./ubinize_ubifs_overlay.cfg),$(call Image/Build,ubi))
	echo "ubinize_ubifs_overlay.cfg is used"
else
...
define Image/mkfs/cpiolzma
	(if [ -f $(PLATFORM_DIR)/image/init ]; then \
		$(CP) $(PLATFORM_DIR)/image/init $(TARGET_DIR)/; \
		cd $(TARGET_DIR); cat $(PLATFORM_DIR)/image/initramfs-base-files.txt | cpio -o -H newc | /usr/bin/lzma >$(BIN_DIR)/rootfs.cpio.lzma; fi;)
endef
