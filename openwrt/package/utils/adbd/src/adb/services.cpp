...
#include "sysdeps.h"
#include "transport.h"
#if ADB_SECURITY_CONNECT
#if ADB_SECURITY_SIGN
#include "rk_sign.h"
#endif
#endif
...
int service_to_fd(const char* name, const atransport* transport) {
    int ret = -1;
#if ADB_SECURITY_CONNECT
    static int root_password = 0;
#if ADB_SECURITY_SIGN
#define SIGN_128_LEN 172
#define RK_SN_MAX_LEN 30
#define VALID_SN_CHAR(sn) ((sn >= '0' && sn <= '9') || (sn >= 'A' && sn <= 'Z') \
		|| (sn >= 'a' && sn <= 'z'))
	FILE * fp = NULL;
	char *serial_n = NULL;
	int num = 0;
	...
	num = fread(buf, 10, 100, fp);
	serial_n = strstr(buf, "androidboot.serialno");
	if (serial_n == NULL) {
		printf("No 'androidboot.serialno' striong!\n");
		return -1;
	}
	serial_n = serial_n + strlen("androidboot.serialno=");
	if (!VALID_SN_CHAR(*serial_n)) {
		printf("Serial number has not been set,skip security check!\n");
		root_password = 1;
	} else {
		if (!strncmp(name, "root:", strlen("root:"))) {
			int i;
			static char sign_buf[512] = {0};
			const char *sign = name + strlen("root:");
			
			for (i = 0; i < RK_SN_MAX_LEN; i++) {
				if (*(serial_n + i) == ' ') {
					*(serial_n + i) = '\0';
					break;
				}
			...
			if (RK_SN_MAX_LEN == i) {
				printf("sn length is too long\n");
				return -1;
			}
			if (strcmp("clean", sign) == 0) {
				printf("clean the sign buf\n");
				memset(sign_buf, 0, 512);
				return -1;
			}
			if (strcat(sign_buf, sign) == NULL) {
				printf("Connect the sign failed\n");
				return -1;
			}
			if (strlen(sign_buf) < SIGN_128_LEN) {
				printf("Length of sign is %d but not 172 please add more\n",
						strlen(sign_buf));
				return -1;
			}
			if (strlen(sign_buf) > SIGN_128_LEN) {
				printf("Length of sign is %d, it's large than 172,please rewrite the sign\n",
						strlen(sign_buf));
				memset(sign_buf, 0, 512);
				return -1;
			}
			if (verify_sign(serial_n, sign_buf)) {
				printf("Sign check success!\n");
				root_password = 1;
			} else {
				printf("Sign check failed!\n");
				return -1;
			}
			memset(sign_buf, 0 ,512);
		}
	}
#elif ADB_SECURITY_RK
    if (!strncmp(name, "root:*#*#777666554443#*#*", 25)) {
            root_password = 1;
    }
#endif
    if (root_password != 1) {
            printf("%s you should input root password first\n", __func__);
            return ret;
    }
#endif
