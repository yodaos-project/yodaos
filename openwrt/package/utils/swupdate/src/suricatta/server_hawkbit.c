...
static struct option long_options[] = {
    ...
    {"gtoken", required_argument, NULL, 'g'},
    {"ttoken", required_argument, NULL, 'l'},
...
server_op_res_t server_send_cancel_reply(channel_t *channel, const int action_id)
{
	...
	channel_data_reply.gateway_token = server_hawkbit.gateway_token;
	channel_data_reply.target_token = server_hawkbit.target_token;
			     ...
			     const char *execution_status, int numdetails, const char *details[])
{
	...
	channel_data.gateway_token = server_hawkbit.gateway_token;
	channel_data.target_token = server_hawkbit.target_token;
...
void suricatta_print_help(void)
{
	fprintf(
	    ...
	    "\t  -g, --gtoken        Set gateway token.\n"
	    "\t  -l, --ttoken        Set target token.\n"
...
server_op_res_t server_start(char *fname, int argc, char *argv[])
{
	...
	optind = 1;
	while ((choice = getopt_long(argc, argv, "t:i:c:u:p:xr:y::w:g:l:",
				     long_options, NULL)) != -1) {
		switch (choice) {
		case 't':
		...
		case 'i':
			...
			break;
		case 'g':
			SETSTRING(server_hawkbit.gateway_token, optarg);
			break;
		case 'l':
			SETSTRING(server_hawkbit.target_token, optarg);
			break;
		case 'c':
