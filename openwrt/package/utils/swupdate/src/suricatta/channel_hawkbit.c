...
static int channel_pack_token(channel_data_t *channel_data, char *token)
{
	if (!channel_data || !token)
		return -1;

	if (channel_data->gateway_token) {
		if (snprintf(token, HTTP_TOKEN_LEN, "Authorization: GatewayToken %s",
			channel_data->gateway_token) < 0) {
			ERROR("pack gatewat token error because of OOM.\n");
			return -1;
		}
	} else if (channel_data->target_token) {
		if (snprintf(token, HTTP_TOKEN_LEN, "Authorization: TargetToken %s",
			channel_data->target_token) < 0) {
			ERROR("pack targetway token error because of OOM.\n");
			return -1;
		}
	} else {
		return -1;
...
static channel_op_res_t channel_post_method(channel_t *this, void *data)
{
	...
	if (!channel_pack_token(channel_data, token)) {
		if ((channel_curl->header = curl_slist_append(
