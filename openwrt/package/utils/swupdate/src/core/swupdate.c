...
static void usage(char *programname)
{
	...
	fprintf(stdout,
		...
		" -C, --CHECK                    : check if upgradable\n"
		" -s, --save <path>              : save file to <path>, do not upgrade\n"
...
int main(int argc, char **argv)
{
	...
	strcpy(main_options, "vhi:e:l:Lcf:p:s:C");
#ifdef CONFIG_MTD
	...
	while ((c = getopt_long(argc, argv, main_options,
		...
		case 'p':
			...
			break;
		case 's':
			swcfg.globals.filesave = 1;
			strncpy(swcfg.filesave_dst, optarg, sizeof(swcfg.filesave_dst));
			break;
		case 'C':
			swcfg.globals.checked = 1;
			break;
		default:
			usage(argv[0]);
			exit(1);
