...
/*
 *	Read MMC Device Key Zone
 */
void read_key_info(void)
{
	#define KEY_BUFFER_SIZE 2048
	#define KEY_ZONE_START 2048 //Block Start Pos
	#define KEY_PART_SIZE 4	    //Block Part Size
	#define KEY_ZONE_MSG_NUMBER 2
	struct mmc* mmc_ptr = NULL;
	mmc_ptr = find_mmc_device(0);
	if(! mmc_ptr)
	{
		printf("Open mmc Device 0 Error.\n");
		return;
	}
	if(mmc_init(mmc_ptr))
	{
		printf("Init mmc device 0 Error.\n");
		return;
	}
	char* buff = malloc(KEY_BUFFER_SIZE);
	for(int i=0; i <= KEY_ZONE_MSG_NUMBER ; i++)
	{

		memset(buff,KEY_BUFFER_SIZE,0);
		blk_dread(mmc_get_blk_desc(mmc_ptr), KEY_ZONE_START+ i*KEY_PART_SIZE , KEY_PART_SIZE ,buff);
		for(int buff_index=0 ; buff_index <KEY_BUFFER_SIZE ;buff_index ++)
		{
			if(buff[buff_index] == 0xff)
			{
		...
		switch(i)
		{
			case 0:
				{
					env_set("rokidseed",buff);
					break;
				}
			case 1:
				{
					env_set("serialno",buff);
					break;
				}
			case 2:
				{
					env_set("devicetypeid",buff);
					break;
				}
			default:
				{
