...
int btusb_open(struct inode *inode, struct file *file)
{
    ...
    /* Don't allow more than one application to open a BTUSB Instance */
    if (p_dev->opened)
    {
        BTUSB_ERR("device already opened\n");
        retval = -EBUSY;
        goto exit;
    }
    /* This btusb instance is opened */
    p_dev->opened = 1;
...
int btusb_release(struct inode *inode, struct file *file)
{
    ...
    /* This btusb instance is not any more opened */
    p_dev->opened = 0;
