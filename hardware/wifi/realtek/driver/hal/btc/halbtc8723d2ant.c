 ...
 *
 * ************************************************************ */
u32	glcoex_ver_date_8723d_2ant = 20180330;
u32	glcoex_ver_8723d_2ant = 0x25;
u32 glcoex_ver_btdesired_8723d_2ant = 0x23;
			       ...
			       IN boolean force_exec, IN boolean rx_gain_en)
{
	...
	btcoexist->btc_set(btcoexist, BTC_SET_BL_BT_LNA_CONSTRAIN_LEVEL, &lna_lvl);
	...
	if (BTC_RSSI_HIGH(wifi_rssi_state) &&
		...
		if (BTC_RSSI_HIGH(wifi_rssi_state3)) {
			if (coex_sta->connect_ap_period_cnt > 0)
				halbtc8723d2ant_ps_tdma(btcoexist, NORMAL_EXEC, TRUE, 121);
			else
				halbtc8723d2ant_ps_tdma(btcoexist, NORMAL_EXEC, TRUE, 119);
		} else {
			if (coex_sta->connect_ap_period_cnt > 0)
				halbtc8723d2ant_ps_tdma(btcoexist, NORMAL_EXEC, TRUE, 120);
			else
