 ...
 *
 * ************************************************************ */
u32	glcoex_ver_date_8723d_1ant = 20180529;
u32	glcoex_ver_8723d_1ant = 0x26;
u32 glcoex_ver_btdesired_8723d_1ant = 0x23;
		...
		IN boolean force_exec, IN u8 type)
{
	...
	case 17:
		halbtc8723d1ant_coex_table(btcoexist, force_exec,
						0x65556555, 0x65556555, break_table,
						select_table);
		break;
	default:
		break;
...
void halbtc8723d1ant_action_bt_inquiry(IN struct btc_coexist *btcoexist)
{
	...
	if ((coex_sta->bt_create_connection) &&
		...
		if ((bt_link_info->a2dp_exist) && (!bt_link_info->pan_exist)) {
			...
			halbtc8723d1ant_coex_table_with_type(btcoexist, NORMAL_EXEC, 16);
		...
		if ((coex_sta->wifi_in_scan_task) || (coex_sta->wifi_is_high_pri_task))
...
void halbtc8723d1ant_action_wifi_multi_port(IN struct btc_coexist *btcoexist)
{
	...
	if (multi_port_type == BTC_MULTIPORT_SCC) {
		if (bt_link_info->a2dp_exist) {
			...
			halbtc8723d1ant_coex_table_with_type(btcoexist, NORMAL_EXEC, 17);
			halbtc8723d1ant_ps_tdma(btcoexist, NORMAL_EXEC, FALSE, 8);
		}
	} else  {
		if ((BT_8723D_1ANT_BT_STATUS_NON_CONNECTED_IDLE ==
...
void halbtc8723d1ant_run_coexist_mechanism(IN struct btc_coexist *btcoexist)
{
	...
	if (coex_sta->freeze_coexrun_by_btinfo && (!coex_sta->is_setupLink)) {
		BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
				       ...
				       IN u8 *tmp_buf, IN u8 length)
{
	...
	if (BT_INFO_SRC_8723D_1ANT_WIFI_FW != rsp_source) {
		...
		if (coex_sta->bt_info_ext & BIT(2) ||
			((coex_sta->bt_create_connection) && (coex_sta->pnp_awake_period_cnt > 0))) {
			coex_sta->cnt_setupLink++;
		...
		if (coex_sta->bt_create_connection) {
			...
			if ((wifi_link) || (wifi_roam) || (coex_sta->wifi_in_scan_task) ||
