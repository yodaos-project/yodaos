...
extern void IRQHandler(struct sdio_func *func);
extern void IRQHandlerF2(struct sdio_func *func);
...
static int bcmsdh_sdmmc_resume(struct device *pdev)
{
	...
	if (func->num == 2) {
		sdio_claim_host(func);
		sdio_claim_irq(func, IRQHandlerF2);
		sdio_release_host(func);
	}

	if (func->num == 1) {
		sdio_claim_host(func);
		sdio_claim_irq(func, IRQHandler);
		sdio_release_host(func);
	}
	if (func->num != 2)
		return 0;
