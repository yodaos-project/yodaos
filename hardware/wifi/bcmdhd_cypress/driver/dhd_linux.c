 ...
 *
 * $Id: dhd_linux.c 681305 2018-01-02 08:19:26Z $
 */
...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	/* no segmented SKB support (Kernel-3.18.y) */     /**added 20170421**/
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (ntoh16(skb->protocol) == ETHER_TYPE_BRCM) {
			...
			if (numpkt != 1) {
#if 0	//remove log for k18 performance issue bin.zhu@rokid.com
...
#ifdef GET_MAC_OTP_MATCH
int cis_chipvendor = 0;
char cis_fw_path[] = "/etc/wifi/43438/cyw43438-7.46.58.15.bin";
char cis_nv_path[] = "/etc/wifi/43438/NB197SM.nvram_20180419_AZ.txt";
#endif
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
	...
	if (cis_chipvendor == 0x81) {
		printf(" HaiHua moduel!\n");
		DHD_ERROR(("Adding firmware and NVRAM path by CIS CY\n"
			"\tfirmware path: %s\n"
			"\tNVRAM path:    %s\n", cis_fw_path, cis_nv_path));
		fw = cis_fw_path;
		nv = cis_nv_path;
	}
#endif
