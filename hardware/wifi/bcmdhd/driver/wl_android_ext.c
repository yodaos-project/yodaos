...
wl_ext_chspec_to_legacy(chanspec_t chspec)
{
	...
	if (wf_chspec_malformed(chspec)) {
		ANDROID_ERROR(("%s: input chanspec (0x%04X) malformed\n", __FUNCTION__,
			chspec));
		return INVCHANSPEC;
	}
	...
	if (CHSPEC_IS20(chspec)) {
		...
		ANDROID_ERROR(("%s: unable to convert chanspec %s (0x%04X) "
			"to pre-11ac format\n", __FUNCTION__,
...
wl_ext_chspec_from_legacy(chanspec_t legacy_chspec)
{
	...
	if (wf_chspec_malformed(chspec)) {
		ANDROID_ERROR(("%s: output chanspec (0x%04X) malformed\n", __FUNCTION__,
			chspec));
		return INVCHANSPEC;
	}
...
wl_ext_get_ioctl_ver(struct net_device *dev, int *ioctl_ver)
{
	...
	if (val != WLC_IOCTL_VERSION && val != 1) {
		ANDROID_ERROR(("%s: Version mismatch, please upgrade. Got %d, expected %d or 1\n",
			__FUNCTION__, val, WLC_IOCTL_VERSION));
		return BCME_VERSION;
	}
	*ioctl_ver = val;
	...
	if (band == IEEE80211_BAND_5GHZ) {
		...
		if (err) {
			if (err != BCME_UNSUPPORTED) {
				ANDROID_ERROR(("%s: bw_cap failed, %d\n", __FUNCTION__, err));
				return err;
			} else {
	...
	if (wf_chspec_valid(chspec)) {
		...
		if (fw_chspec != INVCHANSPEC) {
			if ((err = wl_ext_iovar_setint(dev, "chanspec", fw_chspec)) == BCME_BADCHAN) {
				...
				ANDROID_MSG(dev->name, "channel %d\n", _chan);
			} else if (err) {
				ANDROID_ERROR(("%s: failed to set chanspec error %d\n",
					__FUNCTION__, err));
			} else
				ANDROID_MSG(dev->name, "channel %d, 0x%x\n", channel, chspec);
		} else {
...
wl_ext_pattern_atoh(char *src, char *dst)
{
	...
	if (strncmp(src, "0x", 2) != 0 &&
	    strncmp(src, "0X", 2) != 0) {
		ANDROID_ERROR(("%s: Mask invalid format. Needs to start with 0x\n",
			__FUNCTION__));
		return -1;
	}
	src = src + 2; /* Skip past 0x */
	if (strlen(src) % 2 != 0) {
		ANDROID_ERROR(("%s: Mask invalid format. Needs to be of even length\n",
			__FUNCTION__));
		return -1;
	}
	for (i = 0; *src != '\0'; i++) {
...
wl_ext_keep_alive(struct net_device *dev, char *command, int total_len)
{
	...
	char *buf = NULL;
	char *iovar_buf = NULL;
	int iovar_buf_len = WLC_IOCTL_SMLEN;
	int bytes_written = -1;

	iovar_buf = kzalloc(iovar_buf_len, GFP_KERNEL);
	if (!iovar_buf)
		return -ENOMEM;
	buf = kzalloc(iovar_buf_len, GFP_KERNEL);
	if (!buf) {
		kfree(iovar_buf);
		return -ENOMEM;
	}
...
wl_ext_connect(struct net_device *dev, struct wl_conn_info *conn_info)
{
	...
	*/
	...
	ANDROID_MSG(dev->name, "Connecting with " MACDBG
		" channel (%d) ssid \"%s\", len (%d)\n\n",
...
wl_ext_iapsta_preinit(struct net_device *dev, struct wl_apsta_params *apsta_params)
{
	...
	if (FW_SUPPORTED(dhd, rsdb)) {
		...
		if (apstamode == IDUALAP_MODE || apstamode == ISTAAPAP_MODE ||
			...
			ANDROID_MSG(dev->name, "set rsdb_mode %d\n", rsdb_mode_cfg.config);
			wl_ext_ioctl(dev, WLC_DOWN, NULL, 0, 1);
	...
	ANDROID_MSG(dev->name, "apstamode=%d\n", apstamode);
}
	...
	for (i=0; i<MAX_IF_NUM; i++) {
		...
		if (tmp_if->dev) {
			...
			if (chan) {
				...
				ANDROID_INFO(("[%s][%c-%c%s]: bssid=%pM, chan=%3d, sec=%-11s, SSID=\"%s\"\n",
					tmp_if->ifname, tmp_if->prefix, chan?'E':'D',
					now_if?"*":" ", &bssid, chan, sec, ssid.SSID));
				if (command)
					bytes_written += snprintf(command+bytes_written, total_len,
						"[%s][%c-%c%s]: bssid=%pM, chan=%3d, sec=%-11s, SSID=\"%s\"\n",
						tmp_if->ifname, tmp_if->prefix, chan?'E':'D',
						now_if?"*":" ", &bssid, chan, sec, ssid.SSID);
			} else {
...
wl_ext_if_down(struct wl_apsta_params *apsta_params, struct wl_if_info *cur_if)
{
	...
	ANDROID_MSG(cur_if->ifname, "[%c] Turning off...\n", cur_if->prefix);
...
wl_ext_if_up(struct wl_apsta_params *apsta_params, struct wl_if_info *cur_if)
{
	...
	if (cur_if->channel >= 52 && cur_if->channel <= 148) {
		ANDROID_MSG(cur_if->ifname, "[%c] skip DFS channel %d\n",
			cur_if->prefix, cur_if->channel);
		return 0;
	}

	ANDROID_MSG(cur_if->ifname, "[%c] Turning on...\n", cur_if->prefix);
...
wl_ext_iapsta_disable(struct net_device *dev, char *command, int total_len)
{
	...
	ANDROID_MSG(ifname, "[%c] Disabling...\n", cur_if->prefix);
...
wl_ext_triger_csa(struct wl_apsta_params *apsta_params, struct wl_if_info *cur_if)
{
	...
	if (apsta_params->csa & CSA_DRV_BIT &&
		...
		if (!cur_if->channel) {
			ANDROID_MSG(cur_if->ifname, "[%c] skip channel %d\n",
				cur_if->prefix, cur_if->channel);
			...
			csa_arg.chspec = wl_ext_get_chanspec(apsta_params, cur_if->dev,
				cur_if->channel);
			if (csa_arg.chspec) {
				ANDROID_MSG(cur_if->ifname, "[%c] Trigger CSA to channel %d(0x%x)\n",
	...
	struct net_device *dev, struct wl_if_info *cur_if)
{
	...
	if (target_chan) {
		...
		if (apsta_params->rsdb && tmp_chan &&
			...
			ANDROID_MSG(cur_if->ifname, "[%c] keep on current channel %d\n",
				cur_if->prefix, tmp_chan);
			cur_if->channel = 0;
		} else {
			ANDROID_MSG(cur_if->ifname, "[%c] channel=%d => %s[%c] channel=%d\n",
				cur_if->prefix, cur_if->channel,
	...
	struct net_device *dev, struct wl_if_info *cur_if)
{
	...
	if (target_if) {
		ANDROID_MSG(target_if->ifname, "channel=%d => %s channel=%d\n",
...
wl_ext_enable_iface(struct net_device *dev, char *ifname)
{
	...
	if ((iovar_buf = kzalloc(WLC_IOCTL_SMLEN, GFP_KERNEL)) == NULL) {
		return -ENOMEM;
	}
	...
	if (cur_if->ifmode == IAP_MODE) {
		...
		if (cur_if->hidden > 0) {
			...
			ANDROID_MSG(ifname, "[%c] Broadcast SSID: %s\n",
	...
	OSL_SLEEP(500);
	ANDROID_MSG(ifname, "[%c] enabled with SSID: \"%s\"\n",
	...
	if (cur_if->ifmode == ISTA_MODE) {
		if (event_type == WLC_E_LINK) {
			if (!(flags & WLC_EVENT_MSG_LINK)) {
				ANDROID_MSG(cur_if->ifname, "[%c] Link Down with %pM\n",
					cur_if->prefix, &e->addr);
			} else {
				ANDROID_MSG(cur_if->ifname, "[%c] Link UP with %pM\n",
					cur_if->prefix, &e->addr);
			}
		}
	}
...
wl_ext_iapsta_attach_netdev(struct net_device *net, int ifidx, uint8 bssidx)
{
	...
	ANDROID_MSG(net->name, "ifidx=%d, bssidx=%d\n", ifidx, bssidx);
	if (ifidx < MAX_IF_NUM) {
		cur_if = &apsta_params->if_info[ifidx];
	}
	if (ifidx == 0) {
	...
	int total_len)
{
	...
	if (data) {
		...
		ANDROID_MSG(dev->name, "rsdb_mode %d\n", rsdb_mode_cfg.config);
	} else {
