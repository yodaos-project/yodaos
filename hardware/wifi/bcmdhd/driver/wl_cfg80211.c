	...
	struct cfg80211_connect_params *sme)
{
	...
	*/
	...
	if (cfg->rcc_enabled) {
		WL_MSG(dev->name, "Connecting with " MACDBG " ssid \"%s\", len (%d) with rcc channels\n\n",
			MAC2STRDBG((u8*)(&ext_join_params->assoc.bssid)),
			ext_join_params->ssid.SSID, ext_join_params->ssid.SSID_len);
	} else {
		WL_MSG(dev->name, "Connecting with " MACDBG " ssid \"%s\", len (%d) channel=%d\n\n",
			MAC2STRDBG((u8*)(&ext_join_params->assoc.bssid)),
	...
	*cookie = id;
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) {
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			char *mac_buf = NULL;
			int mac_buf_len = MAX_NUM_OF_ASSOCIATED_DEV * sizeof(struct ether_addr) + sizeof(uint);
			int num_associated = 0;
			struct maclist *assoc_maclist = NULL;

			if ((mac_buf = kzalloc(mac_buf_len, GFP_KERNEL)) == NULL) {
				goto exit;
			}
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (wl_get_mode_by_netdev(cfg, ndev) == WL_MODE_AP) {
		...
		WL_MSG(ndev->name, "wl_notify_connect_status : Invalid mode %d event %d status %d\n",
...
#endif /* CONFIG_ARCH_MSM && TDLS_MGMT_VERSION2 */
{
...
#ifdef WLTDLS
	struct bcm_cfg80211 *cfg;
	tdls_wfd_ie_iovar_t *info = NULL;
	cfg = wl_get_cfg(dev);

	if ((info = kzalloc(sizeof(tdls_wfd_ie_iovar_t), GFP_KERNEL)) == NULL) {
		goto out;
	}
	...
	ret = wldev_iovar_setbuf(dev, "tdls_wfd_ie", info, sizeof(tdls_wfd_ie_iovar_t),
			cfg->ioctl_buf, WLC_IOCTL_MAXLEN, &cfg->ioctl_buf_sync);
