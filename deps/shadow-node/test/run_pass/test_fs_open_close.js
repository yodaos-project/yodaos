...
// Test normal sequence. Open file for reading, doesNotThrow
var fd = fs.openSync(fileName, 'r');
assert.notStrictEqual(fs, null, 'Error opening file: ' + fileName);
fs.closeSync(fd);


// test trying to open not exist file - expecting exception.
assert.throws(function() {
  fs.openSync('not_exist_file', 'r');
...
fs.open('not_exist_file', 'r', function(err, fd) {
  assert.notStrictEqual(err, null, 'Test not exist file - expecting exception');
});
...
assert.throws(function() {
  fs.openSync(null, 123);
}, TypeError, 'Calling fs.openSync with null file path');
assert.throws(function() {
  fs.openSync(process.cwd() + '/run_pass/test_fs_stat.js', 'k');
}, TypeError, 'Calling fs.openSync with invalid flag');
assert.throws(function() {
  fs.openSync(process.cwd() + '/resources/test2.txt', null);
