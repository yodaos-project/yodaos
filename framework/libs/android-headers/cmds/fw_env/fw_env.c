...
#define MMCDEVOFFSET (0xf400000)

//#define CONFIG_ENV_SIZE ENVSIZE(dev_current)
#define CONFIG_ENV_SIZE		(64 << 10)	/* 64K */
...
//NOTE: need replace follow uboot.
...
#else
static int flash_read_buf (int fd, void *buf, size_t count,
			   off_t offset)
{
	size_t processed = 0;	/* progress counter */
	size_t readlen = count;	/* current read length */
	...
	while (processed < count) {
		...
		if(rc > 0)
		{
//			printf("rc = %d,readlen = %d\n",rc,readlen);
			if(rc != readlen) readlen = rc;
		}
		else
		{
			printf("Read error!!!");
...
#else
static int flash_write_buf (int fd, void *buf, size_t count,
			    off_t offset)
{
	size_t write_total = count;
//	size_t processed = 0;
	int rc;

//	while(process < count)
//	{
	if(write(fd,buf,write_total)!= write_total)
	{
		printf("Write error\n");
		return -1;
	}
...
#else
static int flash_write (int fd)
{
	int rc;
	rc = flash_write_buf (fd, environment.image,CONFIG_ENV_SIZE, MMCDEVOFFSET);
	if(rc < 0)
		return rc;
	return 0;
}
...
#else
	rc = flash_read_buf (fd, environment.image, CONFIG_ENV_SIZE,MMCDEVOFFSET);
#endif
...
static int flash_io (int mode)
{
	...
	fd_current = open ("/dev/nand_env", mode);
...
//	printf("crc0 = 0x%08x,ecrc = 0x%08x,size=%d\n",crc0,*environment.crc,ENV_SIZE);
