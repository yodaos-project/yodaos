...
int gen_sha1sum_verify(const char* srFile, char* verifyData)
{
    ...
    for(; totalReadSz < SrcFileSz; totalReadSz += thisReadSz)
    {
        ...
        if(readSz != thisReadSz){
            printf("Want to read 0x%ld, but 0x%ul \n", thisReadSz, readSz);
            ret = __LINE__;
            goto _finish;
        }
...
void calc_sha1sum(uint8_t *buf, uint64_t size, char *res) {
    ...
    sprintf(res, "sha1sum %02x", genSumVal[0]);
    for(; i < 20; ++i) {
        sprintf(res, "%s%02x", res, genSumVal[i]);
    }
    return ;
}
