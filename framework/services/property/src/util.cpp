                  ...
                  gid_t gid, const char *socketcon)
{
    ...
    if (ret != 0 && errno != ENOENT) {
        ALOGE("Failed to unlink old socket '%s'\n", name);
        goto out_close;
    }
#if 0
    filecon = NULL;
...
bool read_file(const char* path, std::string* content) {
    ...
    struct stat sb;
    if (fstat(fd, &sb) == -1) {
        ALOGE("fstat failed for '%s'\n", path);
        return false;
    }
    if ((sb.st_mode & (S_IWGRP | S_IWOTH)) != 0) {
        ALOGE("skipping insecure file '%s' mode %x %x %x\n", path, sb.st_mode, S_IWGRP,  S_IWOTH);
        return false;
    }
...
int write_file(const char* path, const char* content) {
    ...
    if (fd == -1) {
        ALOGE("write_file: Unable to open '%s'\n", path);
        return -1;
    }
    int result = android::base::WriteStringToFd(content, fd) ? 0 : -1;
    if (result == -1) {
        ALOGE("write_file: Unable to write to '%s'\n", path);
    }
    close(fd);
    return result;
}
...
static void find_mtd_partitions(void)
{
    ...
    while (pmtdsize > 0) {
        ...
        if ((r == 4) && (mtdname[0] == '"')) {
            ...
            ALOGI("mtd partition %d, %s\n", mtdnum, mtdname + 1);
            if (mtd_part_count < MAX_MTD_PARTITIONS) {
                strcpy(mtd_part_map[mtd_part_count].name, mtdname + 1);
                mtd_part_map[mtd_part_count].number = mtdnum;
                mtd_part_count++;
            } else {
                ALOGE("too many mtd partitions\n");
            }
        }
...
int mkdir_recursive(const char *pathname, mode_t mode)
{
    ...
    while ((slash = strchr(p, '/')) != NULL) {
        ...
        if ((unsigned int)width > sizeof(buf) - 1) {
            ALOGE("path too long for mkdir_recursive\n");
            return -1;
        }
...
void make_link_init(const char *oldpath, const char *newpath)
{
    ...
    if (ret)
        ALOGE("Failed to create directory %s (%d)\n", buf, errno);

    ret = symlink(oldpath, newpath);
    if (ret && errno != EEXIST)
        ALOGE("Failed to symlink %s to %s (%d)\n", oldpath, newpath, errno);
}
